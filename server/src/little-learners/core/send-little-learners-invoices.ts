import {
    ...
    type LittleLearnersEnrolment,
    type SendLittleLearnersInvoiceParams,
} from 'fizz-kidz'
...
import { getLittleLearnersInvoiceCatalogObjectId } from './little-learners-invoice-config'
import { sendLittleLearnersInvoice } from './send-little-learners-invoice'

import type { InvoiceStatus } from 'square/api'

export async function sendLittleLearnersInvoices(input: SendLittleLearnersInvoiceParams[]) {
    const invoiceStatusMap: InvoiceStatusMap = {}

    try {
        const square = await SquareClient.getInstance()
        const catalogObjectId = getLittleLearnersInvoiceCatalogObjectId()

        await Promise.all(
            input.map(async (invoiceData) => {
                const enrolment = await DatabaseClient.getLittleLearnersEnrolment(invoiceData.id)
                ...
                if (enrolment.invoiceId) {
                    ...
                    if (
                        ...
                        throwTrpcError(
                            ...
                            'unable to find existing Little Learners invoice in square despite enrolment having an invoice id while sending invoice',
                            null,
                            { invoiceData }
                        )
                    }
                    ...
                    if (existingInvoice.status === 'PAID' || existingInvoice.status === 'PARTIALLY_REFUNDED') {
                        throwTrpcError(
                            'PRECONDITION_FAILED',
                            'Cannot send a new Little Learners invoice for an enrolment that has already been paid.',
                            null,
                ...
                const invoice = await sendLittleLearnersInvoice({
                ...
                const updatedEnrolment: Partial<LittleLearnersEnrolment> = {
                    invoiceId: invoice.id,
                }

                await DatabaseClient.updateLittleLearnersEnrolment(invoiceData.id, updatedEnrolment)
        ...
        throwTrpcError('INTERNAL_SERVER_ERROR', 'error occurred sending Little Learners invoice', err)
    }
}
