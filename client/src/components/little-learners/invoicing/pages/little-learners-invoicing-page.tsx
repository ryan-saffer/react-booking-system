...
export function LittleLearnersInvoicingPage() {
    const { currentOrg } = useOrg()

    return <LittleLearnersInvoicingPageContent key={currentOrg ?? 'master'} currentOrg={currentOrg} />
}

function LittleLearnersInvoicingPageContent({ currentOrg }: { currentOrg: ReturnType<typeof useOrg>['currentOrg'] }) {
    const trpc = useTRPC()
                ...
                <div className="w-full max-w-4xl space-y-4">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Little Learners invoices are managed at the program level, not per session.
                            </CardDescription>
                        </CardHeader>
        ...
        return (
            <main className="flex items-center justify-center bg-slate-100 px-4 dashboard-full-screen">
                <Alert variant="destructive" className="max-w-md">
                    <AlertTitle>Unable to load Little Learners invoicing</AlertTitle>
