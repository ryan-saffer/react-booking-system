...
import type { InvoiceStatusMap, LittleLearnersEnrolment } from 'fizz-kidz'
...
type LittleLearnersClientEnrolment = Omit<LittleLearnersEnrolment, 'createdAt' | 'updatedAt'> & {
    createdAt: string | Date
    updatedAt: string | Date
}

export function LittleLearnersInvoicesTable() {
    const trpc = useTRPC()
    ...
    const enrolmentsQuery = useQuery(
        trpc.littleLearners.listEnrolments.queryOptions({
            appointmentTypeId,
        })
    )
    ...
    const enrolments = useMemo(
        () =>
            ((enrolmentsQuery.data || []) as LittleLearnersClientEnrolment[]).sort((a, b) =>
    ...
    const invoiceStatusesQuery = useQuery(
        trpc.littleLearners.retrieveInvoiceStatuses.queryOptions(
    ...
    const sendInvoicesMutation = useMutation(
        trpc.littleLearners.sendInvoices.mutationOptions({
            onSuccess: (result) => {
                queryClient.setQueryData(
                    trpc.littleLearners.retrieveInvoiceStatuses.queryKey({
                        enrolmentIds: enrolments.map((enrolment) => enrolment.id),
                    }),
    ...
    const unenrollMutation = useMutation(
        trpc.littleLearners.unenrollFromLittleLearners.mutationOptions({
            onSuccess: (_, variables) => {
                queryClient.setQueryData(
                    trpc.littleLearners.listEnrolments.queryKey({ appointmentTypeId }),
    ...
    if (!appointmentTypeId || !programName) {
        return <Navigate to="/dashboard/little-learners-invoicing" />
    }
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <h1 className="lilita text-2xl">Little Learners Invoicing</h1>
                        <p className="text-sm text-slate-600">{programName}</p>
                    </div>
                    <Button variant="outline" onClick={() => navigate('/dashboard/little-learners-invoicing')}>
    ...
    if (enrolmentsQuery.isError) {
        return (
            <Alert variant="destructive">
                ...
                <AlertDescription>
                    There was a problem loading the Little Learners enrolments for this program.
                </AlertDescription>
            </Alert>
        )
    }
                ...
                <CardContent>
                    {enrolments.length === 0 ? (
                        <Alert>
                            ...
                            <AlertDescription>
                                No active Little Learners enrolments were found for this program.
                            </AlertDescription>
                        </Alert>
                    ) : (
                        <Table>
