    ...
    return (
        <>
            <div
                ...
                onClick={onClick}
            >
                <div className="h-52 flex-shrink-0 overflow-hidden rounded-t-xl">{image}</div>
                <div slot="content" className="flex h-max flex-grow flex-col justify-between p-8 @container">
                    <div>
                        <p className="font-lilita text-4xl" style={{ color: content.color }}>
                            {content.title}
                        </p>
                    ...
                    <div className="mb-4 mt-6 flex flex-col gap-4 @[18rem]:flex-row">
                        {content.buttons.map((button, idx) => (
                            <HoverTag key={idx} color={button.color} text={button.content} tooltip={button.tooltip} />
                        ))}
                    </div>
                </div>
                <div
