...
import IncursionExpandable from "./incursion-expandable";
...
type Module = "chemical" | "physical" | "light" | "earth" | null;
...
const IncursionModulesSection = ({
  ...
  // ensures that the item expands only after the previous item has finished collapsing.
  const handleModuleClicked = (module: Module) => {
    if (selectedModule === module) {
      ...
      }, 300); // matches expanding animation duration
    }
  };
  ...
  return (
    <>
      <div
        className={cn("grid grid-cols-4 gap-8", { "grid-cols-1": isMobile })}
      >
        <IncursionModule
          position={1}
          isMobile={isMobile}
...
& Sustainability"
      ...
      {/* <IncursionExpandable
        slotName="chemical"
        title="expandable"
        incursions={[{ content: "test", title: "test title" }]}
        images={chemicalImages}
        expanded={selectedModule === "chemical"}
      /> */}
    </>
  );
};
