...
Typed Firestore collection and document references live in `firestore.refs.ts`. Integration modules use descriptive dot-qualified filenames such as `firestore.client.ts` and `firestore.refs.ts`.
...
Keep in `src/features/<feature>/core`:
