...
import type { AcuityTypes, AfterSchoolEnrolment, ScheduleAfterSchoolEnrolmentParams, Studio } from '@fizz-kidz/core'
import {
    AcuityConstants,
    AcuityUtilities,
    capitalise,
    ...
    if (sendPortalEmail) {
        try {
            ...
            await mailClient.sendEmail('afterSchoolParentPortalLink', input.parent.email, {
                ...
                childName: input.child.firstName,
                className: input.className,
                portalUrl: `${getApplicationDomain(env, isUsingEmulator())}/parent-portal/${appointment.id}`,
            })
