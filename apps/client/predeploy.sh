...
# For Apple Pay payments with Square, the cert must be deployed at apps/client/public/.well-known/apple-develop-merchantid-domain-association
...
OUTDIR=apps/client/public/.well-known
mkdir -p "$OUTDIR"

if [ "$GCLOUD_PROJECT" = 'booking-system-6435d' ]; then
    cp apps/client/apple-certs/apple-developer-merchantid-domain-association-dev \
    ...
    cp apps/client/apple-certs/apple-developer-merchantid-domain-association-prod \
        "$OUTDIR/apple-developer-merchantid-domain-association"
