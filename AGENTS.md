...
- `apps/portal/` – Internal React operations portal. Key: `src/app.tsx` (routes), `src/components/root/root.tsx` (providers + tRPC client), `src/utilities/trpc.ts` (tRPC types/client).
...
- `packages/core/` – Shared module (types, constants, utilities, shared logic). Consumed from source by both Portal and server.
- `scripts/` – Operational scripts (e.g., exports, maintenance utilities).

There are five workspaces, declared as `apps/*` and `packages/*`. The directories are `apps/portal`, `apps/server`, `apps/docs`, `apps/website`, and `packages/core`. The apps are named after their directories (`portal`, `server`, `docs`, `website`); the shared module is the scoped package `@fizz-kidz/core`. Apps are deployable and unscoped; shared packages are scoped (`@fizz-kidz/core`). Because no workspace nests inside another, `npm --workspace <name> ...` and `vp add <pkg> --filter <name>` target exactly one package.

## Deployment Boundaries

Two independent deployment systems operate on this repository, and they must not trigger each other.

- **Firebase (GitHub Actions, `.github/workflows/pipeline.yml`)** deploys `portal` (Hosting) and `server` (Functions). Its trigger paths deliberately exclude `apps/docs/**`, `apps/website/**`, and `package-lock.json`, so Netlify-app changes never deploy Firebase.
...
Both `portal` and `server` import the shared module as `@fizz-kidz/core`, resolved by alias rather than by a published package. The `@fizz-kidz/` scope is never published; it exists so an import is instantly identifiable as internal. The alias is declared in four places that must stay in sync: root `vite.config.ts` (Portal and server alias maps), `apps/server/vite.config.ts`, `apps/portal/tsconfig.json`, and `apps/server/tsconfig.json`.

## Start Here: README Index

- Repository overview: [README.md](README.md)
  - Monorepo structure, Portal/server overview, tRPC interaction, setup, dev scripts, deployment with Firebase.
- Portal app: [portal/README.md](apps/portal/README.md)
...
- Invitations & RSVP: Portal flow [portal/src/components/rsvp/README.md](apps/portal/src/components/rsvp/README.md); server lifecycle [server/src/party-bookings/core/rsvp/README.md](apps/server/src/party-bookings/core/rsvp/README.md).
- Holiday Programs (customer booking screen): [portal/src/components/holiday-programs/customer-booking-screen/README.md](apps/portal/src/components/holiday-programs/customer-booking-screen/README.md)
...
4. Run the Portal and Firebase emulators: `npm run dev` (or use `npm run portal` and `npm run server` in separate terminals)
...
- If you add a top-level path that should hit the backend before the SPA renders (for example `/forms/**`), add a Firebase Hosting rewrite for production and a Vite proxy entry for local development. Otherwise the route may work in one environment and silently fall through to the Portal in the other.
- The current durable backend-owned form entrypoints live under `/forms/**`; the Portal `/form` route is an implementation detail behind that redirect layer.
...
- Scope tests while iterating with `vp test --run --project portal` or `--project server`.
...
- Types not found in the Portal: run `vp check` and verify the `@fizz-kidz/core` alias in the root `vite.config.ts` and Portal `tsconfig.json`.
