...
export const sendOutSingleForm = functions
    ...
    .https// .onCall((data: { id: string }, _context: functions.https.CallableContext) => {
    .onRequest(async (req, resp) => {
        const data = req.body as { id: string }

        try {
            const snapshot = await db.doc(`bookings/${data.id}`).get()
            if (snapshot.exists) {
                const booking = snapshot.data() as FirebaseFirestore.DocumentData
