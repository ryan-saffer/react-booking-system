...
export class AcuityClient {
  ...
  private _request<T>(
    ...
    return new Promise((resolve, reject) => {
      this.#acuity.request(
        path,
        options,
        (err: any, _resp: any, result: T | AcuityTypes.Api.Error) => {
  ...
  async searchForAppointments(params: FetchAppointmentsParams) {
    ...
    return result.filter(
      (it) => it.appointmentTypeID === params.appointmentTypeId
    );
  }

  async getAppointmentTypes(input: GetAppointmentTypesParams) {
    let appointmentTypes = await this._request<
      AcuityTypes.Api.AppointmentType[]
    >(`/appointment-types`);
    if (input.category) {
      appointmentTypes = appointmentTypes.filter(
        (it) => it.category === input.category
      );
    }
  ...
  scheduleAppointment(body: ScheduleAppointmentParams) {
    return this._request<AcuityTypes.Api.Appointment>(
  ...
  updateLabel(params: UpdateLabelParams) {
    ...
    const label =
      params.label === "none" ? [] : [{ id: labelMap[params.label] }];
